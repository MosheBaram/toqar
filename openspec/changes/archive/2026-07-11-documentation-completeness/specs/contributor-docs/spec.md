...
Each workspace package and app SHALL have a `README.md` stating what it does, its `@toqar/*` dependencies, and how it is used and tested. This includes the six that currently lack one: `packages/analysis-agent`, `packages/cli`, `packages/collector`, `packages/isolation-suite`, `packages/sdk`, and `apps/web`.
